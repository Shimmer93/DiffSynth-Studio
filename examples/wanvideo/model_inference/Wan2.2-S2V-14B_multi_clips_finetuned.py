...
def speech_to_video(
    ...
    for r in range(num_repeat):
        ...
        print("current clip shape:", len(current_clip), current_clip[0].size)
        current_clip = current_clip[-infer_frames:]
        print("current clip after removing first frame shape:", len(current_clip), current_clip[0].size)
        if r == 0:
...
pipe = WanVideoPipeline.from_pretrained(
    ...
    device="cuda",
    model_configs=[
        ModelConfig(path="models/train/Wan2.2-S2V-14B_faces_float/epoch-0.safetensors"),
...
input_image = Image.open("face.png").convert("RGB").resize((width, height))
...
video_with_audio = speech_to_video(
    ...
    negative_prompt=negative_prompt,
    pose_video_path=None,
    save_path="video_with_audio_face.mp4",
